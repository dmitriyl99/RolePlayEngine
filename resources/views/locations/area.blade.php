    ...
    <div class="block block-rounded text-body-color-light mt-20 bg-primary-dark-op js-appear-enabled animated fadeIn" data-toggle="appear">
        <div class="block-header">
            @if (auth()->user() and auth()->user()->hasRole('admin'))
                <h3 class="block-title text-body-color-light font-w700">{{ $area->name }} <small>Локации</small></h3>
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <div class="d-flex align-items-center">
                                <a href="{{ route('location', ['areaSlug' => $area->slug, 'locationSlug' => $location->slug]) }}" class="font-size-h5 text-primary font-w600">{{ $location->name }}</a>
                                @if (auth()->user() and auth()->user()->hasRole('admin'))
